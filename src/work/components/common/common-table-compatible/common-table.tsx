...
import { TableColumnAlign, TableColumnCompatibleModel } from "../../../models/table/tableColumnCompatible";
import AccessAuthenticationBottomText from "../../app/access-control/bottom-text";
import CommonTableCellNext from "../common-table-next/common-table-cell";
...
const CommonTable = forwardRef<CommonTableRef, CommonTableProps>(function CommonTable(props, ref) {
  ...
  const CommonTableCellComponent = withStyles(commonTableCellComponentStyles, { name: "CommonTableCellComponent" })(
    (cellProps: DxTable.DataCellProps) => {
      const columnModel = cellProps.column as TableColumnCompatibleModel;
      const jsonValue = JSON.parse(cellProps.row as string);
      const cellValues = jsonValue[columnModel.name] ?? [];

      const rowExtraData = _.get(cellProps.row, "row_data", []);

      return (
        <DxTable.Cell {...cellProps}>
          <CommonTableCellNext models={cellValues} columnModel={columnModel} rowExtraData={rowExtraData} />
        </DxTable.Cell>
      );
    },
  );
  ...
  const columnsSortableExtension = useMemo<DxSortingState.ColumnExtension[]>(() => {
    if (tableContainer.tableConfig) {
      ...
      for (const columnId in tableContainer.tableConfig.columns) {
        ...
        const TableColumnCompatibleModel: TableColumnCompatibleModel = tableContainer.tableConfig.columns[columnId];
        if (TableColumnCompatibleModel.sortable) {
          const columnSortableExtensionItem: DxSortingState.ColumnExtension = {
            columnName: TableColumnCompatibleModel.name,
            sortingEnabled: TableColumnCompatibleModel.sortable && !assessControlUnsortable,
          };
