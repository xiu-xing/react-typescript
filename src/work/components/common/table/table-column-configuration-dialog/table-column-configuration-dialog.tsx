...
const TableColumnConfigurationDialog: React.FunctionComponent = () => {
  ...
  // 已选的列
  const [selectedColumns, setSelectedColumns] = useState<TableColumnModel[]>([]);
  // 可选的列选项
  ...
  // checkbox checked
  function addColumn(column: TableColumnModel): void {
  ...
  // 已选列 Item 渲染
  function selectedColumnListItem(key: number, column: TableColumnModel): ReactNode {
    return (
      <SelectedColumn
        key={key}
        column={column}
