...
const TrackTimeline: React.FunctionComponent<GridTimelineProps> = ({ timelineID }) => {
  const classes = useStyles();
  const client = useClient();
  const museClient = useMuseClient();
  const { trackTheme } = TrackContainer.useContainer();
  const { trackTimelineConfigs } = ConfigContainer.useContainer();
  const {
    timelineEntity,
    timelineFilters,
    timelineData,
    updateSelectorFilters,
    updateMultiSelectorFilters,
    setTimelineEntity,
  ...
  async function executeTrackerTimelineQuery(after?: string) {
    setShowNodata(false);
    museClient.sendTrackingEvent("tracker.timeline", {
    ...
    client
      .query<TrackerTimelineQuery, TrackerTimelineQueryVariables>(
        TrackerTimelineDocument,
        {
  ...
  useEffect(() => {
    if (trackTimelineConfigs) {
      const data = TrackerTimelineConfig.fromJSON(trackTimelineConfigs[timelineID]);
      setTrackerTimelineConfig(data);
    }
  }, [timelineID, trackTimelineConfigs]);
        ...
        {trackerTimelineConfig && (
          <div style={{ padding: "8px 24px" }}>
            <FilterInputSelect
              ...
              onSelectFilterChange={updateSelectorFilters}
              onMultiSelectFilterChange={updateMultiSelectorFilters}
            />
          </div>
        )}
        <CommonTimeline
